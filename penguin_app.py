...
# Design the App
st.title('penguin species predicition app')
...
body_mass_g = st.sidebar.slider('Body_Mass_g(in mm)',float(df['body_mass_g'].min()),float(df['body_mass_g'].max()))
...
if st.sidebar.button("Predict"):
  if classifier == 'Support Vector Machine':
    species_type = prediction(svc_model, island, bill_length_input, bill_depth_input, flipper_length_input, body_mass_g, sex)
    score = svc_model.score(X_train, y_train)

  elif classifier =='Logistic Regression':
    species_type = prediction(log_reg, island, bill_length_input, bill_depth_input, flipper_length_input, body_mass_g, sex)
    score = log_reg.score(X_train, y_train)

  else:
    species_type = prediction(rf_clf, island, bill_length_input, bill_depth_input, flipper_length_input, body_mass_g, sex)
